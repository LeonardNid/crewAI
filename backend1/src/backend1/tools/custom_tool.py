...
class FileReaderTool(BaseTool):
    ...
    def _run(self, filename: str):
        """
        Reads the specified file (filename) from the 'templates' directory and returns its content.
        """

        if not os.path.exists(filename):
            return f"Error: The file '{filename}' does not exist in the 'templates' folder."
